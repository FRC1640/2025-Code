package frc.robot.util.pixy2api.links;

import edu.wpi.first.wpilibj.I2C;
import frc.robot.util.pixy2api.Pixy2;
import frc.robot.util.pixy2api.Pixy2.Checksum;
import java.util.Arrays;

/**
 * Java Port of Pixy2 Arduino Library
 *
 * <p>FIRST Robotics WPI API I2C Link to Pixy2
 *
 * <p>CURRENTLY UNTESTED - Use SPI if possible!
 *
 ...
 *     <p>Arduino I2C link class
 */
public class I2CLink implements Link {
  private static final int PIXY_I2C_DEFAULT_ADDR = 0x54;
  private static final int PIXY_I2C_MAX_SEND = 16; // don't send any more than 16 bytes at a time
  ...
  public int send(byte[] buffer, int length) {
    ...
    for (i = 0; i < length; i += PIXY_I2C_MAX_SEND) {
      if (length - i < PIXY_I2C_MAX_SEND) packet = (length - i);
      else packet = PIXY_I2C_MAX_SEND;
