...
def update_diagnostics_labels():
    ...
    temp_value = float(diagnostics["temperature"][:-2])
    if temp_value > 85:
        warning_label.config(text="It's quite toasty in here !", foreground="red")
...
title_label = tb.Label(
    frame_main, text="Orange Pi Diagnostics", font=("Arial", 18, "bold")
)
title_label.pack(pady=10)
temperature_label = tb.Label(
    frame_main, text="Fetching temperature...", font=("Arial", 12)
)
temperature_label.pack(pady=5)
cpu_usage_label = tb.Label(frame_main, text="Fetching CPU usage...", font=("Arial", 12))
cpu_usage_label.pack(pady=5)
memory_usage_label = tb.Label(
    frame_main, text="Fetching memory usage...", font=("Arial", 12)
)
memory_usage_label.pack(pady=5)
disk_usage_label = tb.Label(
    frame_main, text="Fetching disk usage...", font=("Arial", 12)
)
disk_usage_label.pack(pady=5)
warning_label = tb.Label(frame_main, text="", font=("Arial", 14, "bold"))
warning_label.pack(pady=5)
ssh_button = tb.Button(
    frame_main, text="Open SSH Terminal", command=open_ssh_terminal, bootstyle="primary"
)
