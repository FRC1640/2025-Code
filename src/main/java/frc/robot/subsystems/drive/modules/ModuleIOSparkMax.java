...
public class ModuleIOSparkMax implements ModuleIO {
    ...
    private final SimpleMotorFeedforward driveFF = RobotPIDConstants.constructFF(RobotPIDConstants.driveFF);
    ...
    public ModuleIOSparkMax(ModuleInfo id) {
        ...
        steeringEncoder = new ResolverVoltage(id.resolverChannel, DriveConstants.initalSlope, DriveConstants.finalSlope,
                180.0, 90.0, id.angleOffset);
        driveVelocityQueue = SparkOdometryThread.getInstance()
    ...
    public void updateInputs(ModuleIOInputs inputs) {
        ...
        inputs.driveAppliedVoltage = driveSpark.getAppliedOutput() * RobotController.getBatteryVoltage();
        ...
        inputs.steerAppliedVoltage = steerSpark.getAppliedOutput() * RobotController.getBatteryVoltage();
        ...
        inputs.odometryDrivePositionsMeters = drivePositionQueue.stream()
                .mapToDouble((Double value) -> -(value / DriveConstants.driveGearRatio) * DriveConstants.wheelRadius * 2
                        * Math.PI)
                .toArray();
