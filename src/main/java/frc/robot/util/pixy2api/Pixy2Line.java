 ...
 *
 * <p>Defines line tracker for Pixy2
 *
 ...
 *     <p>This file is for defining the Block struct and the Pixy template class version 2.
 *     (TPixy2). TPixy takes a communication link as a template parameter so that all communication
 *     modes (SPI, I2C and UART) can share the same code.
 */
public class Pixy2Line {
  public static final byte LINE_REQUEST_GET_FEATURES = 0x30;
  public static final byte LINE_RESPONSE_GET_FEATURES = 0x31;
  public static final byte LINE_REQUEST_SET_MODE = 0x36;
  public static final byte LINE_REQUEST_SET_VECTOR = 0x38;
  public static final byte LINE_REQUEST_SET_NEXT_TURN_ANGLE = 0x3a;
  public static final byte LINE_REQUEST_SET_DEFAULT_TURN_ANGLE = 0x3c;
  public static final byte LINE_REQUEST_REVERSE_VECTOR = 0x3e;

  public static final byte LINE_GET_MAIN_FEATURES = 0x00;
  public static final byte LINE_GET_ALL_FEATURES = 0x01;

  public static final byte LINE_MODE_TURN_DELAYED = 0x01;
  public static final byte LINE_MODE_MANUAL_SELECT_VECTOR = 0x02;
  public static final byte LINE_MODE_WHITE_LINE = (byte) 0x80;

  // Features
  public static final byte LINE_VECTOR = 0x01;
  public static final byte LINE_INTERSECTION = 0x02;
  public static final byte LINE_BARCODE = 0x04;
  public static final byte LINE_ALL_FEATURES = (LINE_VECTOR | LINE_INTERSECTION | LINE_BARCODE);

  public static final byte LINE_FLAG_INVALID = 0x02;
  public static final byte LINE_FLAG_INTERSECTION_PRESENT = 0x04;

  public static final byte LINE_MAX_INTERSECTION_LINES = 6;
   ...
   *
   * <p>Defaults to getting all available features and waiting for a response
   *
   * <p>Returned data should be retrieved from the cache with {@link #getVectorCache()}, {@link
   * #getIntersectionCache()} or {@link #getBarcodeCache()}
   *
   ...
   * Gets all features from Pixy2
   *
   * <p>Defaults to waiting for a response
   *
   * <p>Returned data should be retrieved from the cache with {@link #getVectorCache()}, {@link
   * #getIntersectionCache()} or {@link #getBarcodeCache()}
   *
        ...
        if (pixy.type == LINE_RESPONSE_GET_FEATURES) {
          ...
          for (offset = 0, res = 0; pixy.length > offset; offset += fsize + 2) {
            ...
            if (ftype == LINE_VECTOR) {
              ...
              intersections =
                  new Intersection
                      [(int) Math.floor(fdata.length / (4 + (4 * LINE_MAX_INTERSECTION_LINES)))];
   ...
   *
   * <p>{@link #getFeatures(byte, byte, boolean)} must be executed first to get the data actual from
   * Pixy2
   *
   * @return Pixy2 Lines
   */
  ...
  public byte setMode(int mode) {
    ...
    pixy.sendPacket();
    if (pixy.receivePacket() == 0
        && pixy.type == Pixy2.PIXY_TYPE_RESPONSE_RESULT
        && pixy.length == 4) {
      res =
          ((pixy.buffer[3] & 0xff) << 24)
              | ((pixy.buffer[2] & 0xff) << 16)
              | ((pixy.buffer[1] & 0xff) << 8)
              | (pixy.buffer[0] & 0xff);
