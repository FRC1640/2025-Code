 ...
 *
 * <p>Defines Color Connected Components tracker for Pixy2
 *
...
public class Pixy2CCC {
  ...
  public static final byte CCC_RESPONSE_BLOCKS = 0x21;
  public static final byte CCC_REQUEST_BLOCKS = 0x20;
  ...
  // PIXY_SIG1 | PIXY_SIG5
  public static final byte CCC_SIG1 = 0x01;
  public static final byte CCC_SIG2 = 0x02;
  public static final byte CCC_SIG3 = 0x04;
  public static final byte CCC_SIG4 = 0x08;
  public static final byte CCC_SIG5 = 0x10;
  public static final byte CCC_SIG6 = 0x20;
  public static final byte CCC_SIG7 = 0x40;
  public static final byte CCC_COLOR_CODES = (byte) 0x80;
   ...
   *
   * <p>Defaults to waiting for a response, getting blocks from all signatures and a maximum of all
   * 256 blocks
   *
   * <p>Returned data should be retrieved from the cache with {@link #getBlockCache()}
   *
   ...
   * Gets signature {@link Block}s from Pixy2
   *
   * <p>Defaults to getting blocks from all signatures and a maximum of all 256 blocks
   *
   * <p>Returned data should be retrieved from the cache with {@link #getBlockCache()}
   *
   ...
   * Gets signature {@link Block}s from Pixy2
   *
   * <p>Defaults to getting a maximum of all 256 blocks
   *
   * <p>Returned data should be retrieved from the cache with {@link #getBlockCache()}
   *
   * @param wait Whether to wait for Pixy2 if data is not available
      ...
      if (pixy.receivePacket() == 0) {
        if (pixy.type == CCC_RESPONSE_BLOCKS) {
          ...
          for (int i = 0; i + 13 < pixy.length; i += 14) {
            Block b =
                new Block(
                    ...
                    (pixy.buffer[i + 12] & 0xff),
                    (pixy.buffer[i + 13] & 0xff));
            blocks.add(b);
          }
          ...
          if (pixy.buffer[0] == Pixy2.PIXY_RESULT_BUSY) {
            if (!wait) return Pixy2.PIXY_RESULT_BUSY; // New data not available yet
   ...
   *
   * <p>{@link #getBlocks(boolean, int, int)} must be executed first to get the data actual from
   * Pixy2
   *
  ...
  public static class Block {
     ...
     */
    public Block(
        int signature, int x, int y, int width, int height, int angle, int index, int age) {
