package frc.robot.util.pixy2api;

import frc.robot.util.pixy2api.links.I2CLink;
import frc.robot.util.pixy2api.links.Link;
import frc.robot.util.pixy2api.links.SPILink;
import frc.robot.util.pixy2api.links.UARTLink;
 ...
 *
 * <p>Interfaces with the Pixy2 over any provided, compatible link
 *
 * <p>https://github.com/PseudoResonance/Pixy2JavaAPI
 *
 ...
 *     <p>This file is part of Pixy CMUcam5 or "Pixy" for short
 *     <p>All Pixy source code is provided under the terms of the GNU General Public License v2
 *     (http://www.gnu.org/licenses/gpl-2.0.html). Those wishing to use Pixy source code, software
 *     and/or technologies under different licensing terms should contact us at cmucam@cs.cmu.edu.
 *     Such licensing terms are available for all portions of the Pixy codebase presented here.
 *     <p>end license header
 *     <p>Main Pixy template class. This class takes a link class and uses it to communicate with
 *     Pixy over I2C, SPI, UART or USB using the Pixy packet protocol.
 */
public class Pixy2 {
  ...
  // Packet types
  public static final byte PIXY_TYPE_REQUEST_CHANGE_PROG = 0x02;
  public static final byte PIXY_TYPE_REQUEST_RESOLUTION = 0x0c;
  public static final byte PIXY_TYPE_RESPONSE_RESOLUTION = 0x0d;
  public static final byte PIXY_TYPE_REQUEST_VERSION = 0x0e;
  public static final byte PIXY_TYPE_RESPONSE_VERSION = 0x0f;
  public static final byte PIXY_TYPE_RESPONSE_RESULT = 0x01;
  public static final byte PIXY_TYPE_RESPONSE_ERROR = 0x03;
  public static final byte PIXY_TYPE_REQUEST_BRIGHTNESS = 0x10;
  public static final byte PIXY_TYPE_REQUEST_SERVO = 0x12;
  public static final byte PIXY_TYPE_REQUEST_LED = 0x14;
  public static final byte PIXY_TYPE_REQUEST_LAMP = 0x16;
  public static final byte PIXY_TYPE_REQUEST_FPS = 0x18;

  // Return result values
  public static final byte PIXY_RESULT_OK = 0;
  public static final byte PIXY_RESULT_ERROR = -1;
  public static final byte PIXY_RESULT_BUSY = -2;
  public static final byte PIXY_RESULT_CHECKSUM_ERROR = -3;
  public static final byte PIXY_RESULT_TIMEOUT = -4;
  public static final byte PIXY_RESULT_BUTTON_OVERRIDE = -5;
  public static final byte PIXY_RESULT_PROG_CHANGING = -6;

  // RC-servo values
  public static final int PIXY_RCS_MIN_POS = 0;
  public static final int PIXY_RCS_MAX_POS = 1000;
  public static final int PIXY_RCS_CENTER_POS = ((PIXY_RCS_MAX_POS - PIXY_RCS_MIN_POS) / 2);
  ...
  public int init(int argument) {
    ...
    if (ret >= 0) {
      ...
      for (long t = System.currentTimeMillis(); System.currentTimeMillis() - t < 5000; ) {
        // Gets version and resolution to check if communication is successful and stores for future
        // use
        if (getVersion() >= 0) {
          getResolution();
  ...
  private byte getSync() {
    ...
    for (i = attempts = cprev = 0; true; i++) {
      ...
      if (i >= 4) {
        if (attempts >= 4) return PIXY_RESULT_ERROR;
        try {
  ...
  protected int receivePacket() {
    ...
    if (m_cs) {
      ...
      if (csSerial != csCalc.getChecksum()) return PIXY_RESULT_CHECKSUM_ERROR;
    } else {
      // Non-checksum sync
  ...
  public byte changeProg(char[] prog) {
    ...
    while (true) {
      ...
      for (int i = 0; i < PIXY_MAX_PROGNAME; i++) {
        if (i < prog.length) bufferPayload[i] = (byte) prog[i];
        else bufferPayload[i] = Character.MIN_VALUE;
      }
      ...
      } else return PIXY_RESULT_ERROR; // Some kind of bitstream error
      try {
  ...
  public int getVersion() {
    ...
    if (receivePacket() == 0) {
      ...
      } else if (type == PIXY_TYPE_RESPONSE_ERROR) return PIXY_RESULT_BUSY;
    }
  ...
  public byte setServos(int pan, int tilt) {
    ...
    pan =
        (pan >= PIXY_RCS_MAX_POS
            ? PIXY_RCS_MAX_POS
            : (pan <= PIXY_RCS_MIN_POS ? PIXY_RCS_MIN_POS : pan));
    tilt =
        (tilt >= PIXY_RCS_MAX_POS
            ? PIXY_RCS_MAX_POS
            : (tilt <= PIXY_RCS_MIN_POS ? PIXY_RCS_MIN_POS : tilt));
   ...
   *
   * <p>Use 1 to indicate on, 0 to indicate off
   *
